...
```{r example-analysis_funding_forecast, fig.retina = 2, fig.width = 12, fig.asp = 0.618, fig.align = "center", out.width = "90%"}
...
```{r example-analysis_portfolio_scores, fig.retina = 2, fig.width = 12, fig.asp = 0.418, fig.align = "center", out.width = "90%"}
...
#' @param top_n number of observation on the plot
#' @return a list with data, aka a Tibble with donor and assigned segment plus 
...
analysis_donor_segmentation <- function(flows, recipient_name,
  ...
  label_data_top_donors <- seg |>
    dplyr::filter(!is.na(total)) |>
    dplyr::arrange(desc(total)) |>
    dplyr::slice_head(n = top_n) |>
    dplyr::mutate(donor = stringr::str_wrap(donor, width = 45))
  
  plot <- ggplot2::ggplot(res2, ggplot2::aes(x = Strategic_Priority,
  ...
  result <- list(data = seg, plot = plot)
  return(result)
}
```
...
analysis_competitive_intel_matrix <- function(flows,
  ...
  plot <- ggplot2::ggplot(market,
                          ggplot2::aes(x = market_share, y = growth_differential)) +
    ggplot2::geom_point(ggplot2::aes(size = our_funding, color = sector), alpha = 0.7) +
    ggplot2::geom_text(
...
fusen::inflate(flat_file = "dev/flat_dev_4_analysis.Rmd", vignette_name = "Doc-4-Analysis")
```
